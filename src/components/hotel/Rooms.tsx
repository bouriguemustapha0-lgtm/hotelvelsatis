        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-12 md:mb-20">
          <div>
            <p className="eyebrow flex items-center gap-3">
              <span className="gold-rule" /> {t("rooms.eyebrow")}
            </p>
            <h2 className="mt-6 font-display text-4xl md:text-6xl leading-[1.05] max-w-2xl">
              {t("rooms.title1")} <em className="italic text-gold">{t("rooms.titleEm")}</em>
            </h2>
          </div>
          ...
          {ROOMS.map((room, i) => (
            ...
            >
              <div className="relative aspect-[4/3] overflow-hidden">
                <img
                  src={room.image}
                  alt={t(`rooms.items.${room.id}.name`)}
                ...
                <div className="absolute top-5 left-5 bg-cream/95 backdrop-blur px-3 py-1.5 text-[0.65rem] tracking-luxe uppercase text-ink">
                  {t(`rooms.beds.${room.bed}`)}
                </div>
              </div>

              <div className="p-6 md:p-9">
                <div className="flex items-baseline justify-between gap-3">
                  <h3 className="min-w-0 font-display text-xl md:text-3xl">{t(`rooms.items.${room.id}.name`)}</h3>
          ...
          >
            ...
            >
              ...
              <div className="aspect-[4/5] md:aspect-auto md:h-full">
                <img src={active.image} alt={t(`rooms.items.${active.id}.name`)} className="h-full w-full object-cover" />
              </div>
              <div className="p-8 md:p-10">
                <p className="eyebrow">{t(`rooms.beds.${active.bed}`)}</p>
                <h3 className="mt-3 font-display text-3xl md:text-4xl">{t(`rooms.items.${active.id}.name`)}</h3>
                <p className="mt-4 text-foreground/75 leading-relaxed">{t(`rooms.items.${active.id}.description`)}</p>

                <div className="mt-6">
                  <p className="eyebrow mb-4">{t("rooms.inEveryRoom")}</p>
                  <ul className="space-y-2.5">
                    {active.amenities.map((a) => (
                      <li key={a} className="flex items-center gap-3 text-sm text-foreground/80">
                        <Check size={14} className="text-gold shrink-0" /> {t(`rooms.amenities.${a}`)}
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div className="mt-8 flex items-end justify-between border-t hairline pt-6">
                  <div>
                    ...
                    <p className="text-[0.65rem] tracking-luxe uppercase text-muted-foreground">{t("rooms.perNight")}</p>
                  </div>
                  <a
                    href="#contact"
                    onClick={() => setActive(null)}
